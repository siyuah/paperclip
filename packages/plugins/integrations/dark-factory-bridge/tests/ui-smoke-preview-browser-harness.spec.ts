...
describe("UI smoke preview browser harness", () => {
  it("builds a standalone HTML harness with all preview scenarios and boundary fields", () => {
    ...
    expect(html).toContain("远程提供方试运行防护");
    expect(html).toContain("执行试运行");
    expect(html).toContain("试运行回执");
    expect(html).toContain("是否联系提供方");
    ...
    expect(html).toContain("Dark Factory 日志仍是事实来源");
