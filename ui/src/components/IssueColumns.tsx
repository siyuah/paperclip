...
const issueColumnDescriptions: Record<InboxIssueColumn, string> = {
  status: "左侧显示事项状态标记。",
  id: "事项编号，例如 PAP-1009。",
  assignee: "已分配的代理或看板用户。",
  project: "带项目颜色的关联项目标签。",
  workspace: "该事项使用的执行或项目工作区。",
  parent: "父事项编号和标题。",
  labels: "事项标签。",
  updated: "最近可见活动时间。",
};
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-[300px] rounded-xl border-border/70 p-1.5 shadow-xl shadow-black/10">
        ...
        >
          恢复默认
          <span className="ml-auto text-xs text-muted-foreground">状态、ID、最近更新</span>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
          ...
          return (
            <span key={column} className="min-w-0 truncate text-xs text-muted-foreground" title={parentTitle ?? undefined}>
              {parentIdentifier ? (
                ...
                <span className="italic">子事项</span>
              )}
            </span>
          );
        }
