  ...
  async function handleStep2Next() {
    ...
    try {
      if (adapterType === "opencode_local") {
        ...
        if (!selectedModelId) {
          setError(
            "OpenCode 需要使用 provider/model 格式显式选择模型。"
          );
          return;
        }
        ...
        if (!discoveredModels.some((entry) => entry.id === selectedModelId)) {
          setError(
            discoveredModels.length === 0
              ? "未发现 OpenCode 模型。请运行 `opencode models` 并完成提供方认证。"
  ...
  async function handleUnsetAnthropicApiKey() {
    ...
    try {
      ...
      if (result?.status === "fail") {
        setError(
          "已在适配器配置中清空 ANTHROPIC_API_KEY 后重试，但环境测试仍失败。"
        );
      }
    } catch (err) {
      setError(
              ...
              {step === 3 && (
                <div className="space-y-5">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <div>
                      <h3 className="font-medium">给它安排一件事</h3>
                      <p className="text-xs text-muted-foreground">
                        给你的代理一个小任务开始，例如修复 bug、研究问题或编写脚本。
                      </p>
                    </div>
                  </div>
                  <div>
                ...
                <div className="space-y-5">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        一切已设置完毕。现在启动会创建初始任务、唤醒代理，并打开事项。
                      </p>
                    </div>
                  </div>
