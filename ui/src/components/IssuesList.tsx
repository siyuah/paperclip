      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="min-w-0 border border-border bg-background px-3 py-2 text-sm lg:w-72">
          {target && targetIssue ? (
            ...
            <div className="text-sm font-medium text-foreground">没有活跃子事项</div>
          ) : summary.doneCount === summary.totalCount ? (
            <div className="text-sm font-medium text-foreground">所有子事项已完成</div>
          ) : (
            <div className="text-sm font-medium text-foreground">没有可操作的子事项</div>
          )}
        </div>
      </div>
    </div>
  ...
  const groupedContent = useMemo(() => {
    ...
    if (viewState.groupBy === "parent") {
      ...
      return Object.keys(groups)
        ...
        .map((key) => ({
          key,
          label: key === "__no_parent" ? "无父事项" : (issueTitleMap.get(key) ?? key.slice(0, 8)),
          items: groups[key]!,
        }));
    }
    // assignee
                ...
                const renderIssueRow = (issue: Issue, depth: number) => {
                  ...
                  const visibleBlockerChips = unresolvedVisibleBlockers
                    .map((blockerId) => {
                      ...
                      return { blockerId, chipLabel: `被 ${label} 阻断${blockerStepSuffix}` };
                    })
                  ...
                  const additionalVisibleBlockerLabel = additionalVisibleBlockerCount > 0
                    ? ` ... 另有 ${additionalVisibleBlockerCount} 个`
                    : "";
              ...
              <p className="text-xs text-muted-foreground">
                {isLoadingMoreIssues
                  ...
                  : remainingIssueRowCount > 0
                    ? `正在渲染 ${Math.min(renderedIssueRowLimit, filtered.length)} / ${filtered.length} 个事项`
                    : "滚动加载更多事项"}
              </p>
            </div>
          )}
