        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <CardDescription className="text-xs mt-0.5">
              输入 <span className="font-mono">{formatTokens(totalInputTokens)}</span>
              {" · "}
              输出 <span className="font-mono">{formatTokens(totalOutputTokens)}</span>
              {(totalApiRuns > 0 || totalSubRuns > 0) && (
                <span className="ml-1.5">
                  ·{" "}
                  {totalApiRuns > 0 && `约 ${totalApiRuns} 次 API`}
                  {totalApiRuns > 0 && totalSubRuns > 0 && " / "}
                  {totalSubRuns > 0 && `约 ${totalSubRuns} 次订阅`}
                  {"运行"}
                </span>
              )}
            </CardDescription>
        ...
        {hasBudget && (
          <div className="space-y-3">
            ...
            <QuotaBar
              ...
              rightLabel={`约 ${formatCents(Math.round(weeklyBudgetShare))} / 周`}
          ...
          <>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                ...
                {totalSubTokens > 0 && (
                  <>
                    <span className="font-mono text-foreground">{formatTokens(totalSubTokens)}</span> 总量
                    {" · "}
                  </>
                )}
                输入 <span className="font-mono text-foreground">{formatTokens(totalSubInputTokens)}</span>
                {" · "}
                输出 <span className="font-mono text-foreground">{formatTokens(totalSubOutputTokens)}</span>
              </p>
              {subSharePct > 0 && (
                <>
                  ...
                  <div key={`${row.provider}:${row.model}`} className="space-y-1.5">
                    ...
                    <div className="relative h-2 w-full border border-border overflow-hidden">
                      <div
                        ...
                        title={`占提供方 token 的 ${Math.round(tokenPct)}%`}
                      />
                      ...
                      <div
                        ...
                        style={{ width: `${costPct}%` }}
                        title={`占提供方成本的 ${Math.round(costPct)}%`}
                      />
                    </div>
                  </div>
                );
                      ...
                      return (
                        <div key={qw.label} className="space-y-1">
                          ...
                          {qw.detail ? (
                            ...
                            <p className="text-xs text-muted-foreground">
                              重置时间 {new Date(qw.resetsAt).toLocaleDateString("zh-CN", { month: "short", day: "numeric" })}
                            </p>
                          ) : null}
                        </div>
                      );
