...
export function InstanceSettings() {
  ...
  if (heartbeatsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">正在加载调度心跳...</div>;
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">调度心跳</h1>
        </div>
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span><span className="font-semibold text-foreground">{activeCount}</span> 个活跃</span>
        <span><span className="font-semibold text-foreground">{disabledCount}</span> 个已禁用</span>
        ...
        {anyEnabled && (
          <Button
            ...
            onClick={() => {
              if (!window.confirm(`要禁用全部 ${enabledCount} 个已启用代理的定时心跳吗？`)) {
                return;
              }
