...
const CONFLICT_KIND_LABELS: Record<ConflictItem["kind"], string> = {
  agent: "代理",
  project: "项目",
  issue: "事项",
  skill: "技能",
...
function renderImportFileExtra(node: FileTreeNode, checked: boolean, renameMap: Map<string, string>) {
  ...
  const actionBadge = node.action ? (
    <span className={cn(
      ...
      {checked ? (ACTION_LABELS[node.action] ?? node.action) : ACTION_LABELS.skip}
    </span>
  ) : null;
            ...
            return (
              ...
              >
                ...
                <span className={cn(
                  ...
                  {CONFLICT_KIND_LABELS[item.kind]}
                </span>
...
export function CompanyImport() {
  ...
  const localZipHelpText =
    "上传直接从 Paperclip 导出的 .zip 压缩包。由 Finder、Explorer 或其他压缩工具重新打包的归档可能无法正确导入。";
  ...
  const previewMutation = useMutation({
    ...
    if (!source) throw new Error("未配置导入来源。");
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: "预览失败",
        body: err instanceof Error ? err.message : "导入预览失败。",
      });
    },
  });
  ...
  const importMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: "导入完成",
        body: `${result.company.name}: 已处理 ${result.agents.length} 个代理。`,
      });
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Download} message="请选择要导入到的公司。" />;
  }
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            选择 GitHub 仓库，或上传本地 Paperclip 压缩包。
          </p>
        </div>
        ...
        <div className="grid gap-2 md:grid-cols-2">
          {(
            [
              ...
              { key: "local", icon: Upload, label: "本地压缩包" },
            ] as const
        ...
        {sourceMode === "local" ? (
          <div className="rounded-md border border-dashed border-border px-3 py-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {localPackage && (
                <span className="text-xs text-muted-foreground">
                  {localPackage.name}，共 {Object.keys(localPackage.files).length} 个文件
                </span>
              )}
            </div>
            {!localPackage && (
          ...
          <Field
              label="GitHub 地址"
            hint="仓库 tree 路径，或指向 COMPANY.md 的 blob URL（例如 github.com/owner/repo/tree/main/company）。"
          >
            <input
        ...
        <Field label="目标" hint="导入到当前公司，或创建一个新公司。">
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
            value={targetMode}
            onChange={(e) => {
        ...
        <Field
          label="冲突策略"
          hint="看板导入可以对匹配的公司内容执行重命名、跳过或替换。"
        >
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
          ...
          >
            ...
            <option value="replace">替换已有内容</option>
          </select>
        </Field>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {previewMutation.isPending ? "正在预览..." : "预览导入"}
          </Button>
        </div>
      </div>
