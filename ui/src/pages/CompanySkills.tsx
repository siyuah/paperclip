...
function formatProjectScanSummary(result: CompanySkillProjectScanResult) {
  ...
  return `${parts.join("，")}，扫描 ${result.scannedWorkspaces} 个工作区。`;
}
    ...
    <div className="min-w-0">
        ...
        <div className="mt-4 space-y-3 border-t border-border pt-4 text-sm">
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
            <div className="flex items-center gap-2">
              <span className="text-[11px] tracking-[0.18em] text-muted-foreground">来源</span>
            ...
            {detail.sourceType === "github" && (
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-[11px] tracking-[0.18em] text-muted-foreground">固定版本</span>
                <span className="font-mono text-xs">{currentPin ?? "未跟踪"}</span>
                {updateStatus?.trackingRef && (
                  <span className="text-xs text-muted-foreground">正在跟踪 {updateStatus.trackingRef}</span>
                )}
                <Button
                  variant="ghost"
                  size="sm"
            ...
            <div className="flex items-center gap-2">
              <span className="text-[11px] tracking-[0.18em] text-muted-foreground">键</span>
              <span className="font-mono text-xs">{detail.key}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-[11px] tracking-[0.18em] text-muted-foreground">模式</span>
              <span>{detail.editable ? "可编辑" : "只读"}</span>
            </div>
          </div>
          <div className="flex flex-wrap items-start gap-x-3 gap-y-1">
            <span className="text-[11px] tracking-[0.18em] text-muted-foreground">使用者</span>
            {usedBy.length === 0 ? (
      ...
      <div className="min-h-[560px] px-5 py-5">
        {fileLoading ? (
          ...
          <div className="text-sm text-muted-foreground">请选择一个文件查看。</div>
        ) : editMode && file.editable ? (
          file.markdown ? (
            <MarkdownEditor
              value={draft}
