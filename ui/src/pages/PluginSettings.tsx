...
function displayPluginStatus(status: string): string {
...
export function PluginSettings() {
  ...
  const displayStatus = displayPluginStatus(plugin.status);
  const statusVariant =
    plugin.status === "ready"
      ? "default"
      : plugin.status === "error"
                      ...
                      {plugin.categories.length > 0 ? (
                        plugin.categories.map((category) => (
                          <Badge key={category} variant="outline" className="capitalize">
                            {displayPluginCategory(category)}
                          </Badge>
                        ))
                      ) : (
            ...
            <section className="space-y-4 rounded-md border border-border/60 bg-card/40 p-5">
              ...
              {hasCustomSettingsPage ? (
                ...
                <div className="rounded-md border border-border/60 bg-background/60 px-4 py-3 text-sm">
                  <p className="font-medium text-foreground">请从公司环境中配置此插件。</p>
                  <p className="mt-1 text-muted-foreground">
                    {driverLabel || "此插件"} 会在公司环境中注册运行时环境设置，让凭据保持公司级作用域，而不是实例级全局配置。
                  </p>
                  <div className="mt-3">
                    ...
                    <>
                      <div>
                        ...
                        {dashboardData.worker ? (
                          <div className="grid grid-cols-2 gap-3 text-sm">
                            <div className="flex justify-between gap-3 rounded-md border border-border/60 bg-background/50 px-3 py-2">
                              ...
                              <Badge variant={dashboardData.worker.status === "running" ? "default" : "secondary"}>
                                {displayPluginStatus(dashboardData.worker.status)}
                              </Badge>
                            </div>
                            <div className="flex justify-between gap-3 rounded-md border border-border/60 bg-background/50 px-3 py-2">
                              ...
                              <>
                                <div className="flex justify-between col-span-2">
                                  ...
                                  <span className="text-xs">
                                    连续 {dashboardData.worker.consecutiveCrashes} 次 / 总计 {dashboardData.worker.totalCrashes} 次
                                  </span>
                                </div>
                                {dashboardData.worker.lastCrashAt && (
                                  <div className="flex justify-between col-span-2">
                                    <span className="text-muted-foreground">最后崩溃</span>
                          ...
                          <p className="text-sm text-muted-foreground italic">尚未注册 Worker 进程。</p>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {dashboardData.recentJobRuns.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">暂无任务运行记录。</p>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {dashboardData.recentWebhookDeliveries.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">暂无 webhook 投递记录。</p>
                        )}
                      </div>
                    ...
                    <div className="space-y-4 text-sm">
                      <div className="flex items-center justify-between">
                        ...
                        <Badge variant={healthData.healthy ? "default" : "destructive"}>
                          {displayPluginStatus(healthData.status)}
                        </Badge>
                      </div>
