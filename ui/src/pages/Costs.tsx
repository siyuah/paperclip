    ...
    <Card>
      ...
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricTile
          ...
          subtitle="所选期间内借记减去贷记"
          icon={ReceiptText}
        />
        <MetricTile
          label="预估"
          value={formatCents(estimatedDebitCents)}
          subtitle="尚未以发票为准的预估借记"
          icon={Coins}
        />
      </CardContent>
    </Card>
...
export function Costs() {
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={DollarSign} message="选择公司以查看成本。" />;
  }
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              ...
              subtitle={`${formatTokens(inferenceTokenTotal)} 个 token，来自请求级事件`}
              icon={DollarSign}
            />
            <MetricTile
              label="预算"
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ? `${budgetData?.pausedAgentCount ?? 0} 个代理已暂停 · ${budgetData?.pausedProjectCount ?? 0} 个项目已暂停`
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        <TabsList variant="line" className="justify-start">
          ...
          <TabsTrigger value="providers">提供方</TabsTrigger>
          ...
          {showCustomPrompt ? (
            ...
            <>
                  ...
                  <CardContent className="space-y-4 px-5 pb-5 pt-2">
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      ...
                      <div className="border border-border px-4 py-3 text-right">
                        <div className="text-[11px] tracking-[0.14em] text-muted-foreground">用量</div>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">按代理</CardTitle>
                    <CardDescription>所选期间内每个代理消耗的用量。</CardDescription>
                  </CardHeader>
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) ? (
                                  <div className="text-xs text-muted-foreground">
                                    {row.apiRunCount > 0 ? `${row.apiRunCount} 次 API` : "0 次 API"}
                                    {" · "}
                                    {row.subscriptionRunCount > 0
                                      ? `${row.subscriptionRunCount} 次订阅`
                                      : "0 次订阅"}
                                  </div>
                                ) : null}
                              </div>
                ...
                <div className="space-y-4">
                  <Card>
                    ...
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">暂无归属到项目的运行成本。</p>
                      ) : (
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage="暂无财务事件。计费方发票或抵扣到账后，可添加账户级费用。" />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">预算控制台</CardTitle>
                  <CardDescription>
                    代理和项目的硬停止支出限制。提供方订阅配额保持独立，并显示在“提供方”下。
                  </CardDescription>
                </CardHeader>
                    ...
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold">
                          {scopeType === "company" ? "公司预算" : scopeType === "agent" ? "代理预算" : "项目预算"}
                        </h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? "公司级月度策略。"
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                      尚无预算策略。可从代理和项目详情页设置预算，或使用现有公司月度预算控制。
                    </CardContent>
                  </Card>
                ) : null}
              </div>
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">此期间没有成本事件。</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">此期间没有可计费事件。</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      ...
                      <CardDescription>按收费或入账方分组的账户级财务事件。</CardDescription>
                    </CardHeader>
